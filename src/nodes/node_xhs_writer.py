...
你的任务是将今日 AI 圈资讯转化为 3-5 张具有强烈视觉冲击力的数字杂志"卡片"，并配上一段高质量、结构化的小红书配套文案。

对于小红书正文（content 字段），你的内容结构必须包含以下四个固定模块：
1. 🚨 重磅事件：一句话总结今日最大的行业动向或事件。
2. 📰 快讯速览：挑选 3-4 条核心资讯，用简短利落的语言概括。
3. 🐙 GitHub 星榜单：推荐 5 个具备高 Star、有趣且极具价值的 AI 相关的优质开源项目（附带一句话功能亮点介绍，保持简练）。
4. 🔑 今日关键词：提炼 2-3 个代表今日资讯的核心词汇。

对于图片卡片（cards 字段），你的设计基调是：
...
2) 正文必须不超过 1000 个汉字（含标点），超出会导致发布失败（注意星榜单介绍一定要短促精炼才能放得下）。
3) 标题与正文【绝对禁止】出现任何 Markdown 符号（如 **、#、`、>、-、*）。请完全使用纯文本与 Emoji 配合进行排版（如用 🔸 替代列表的 -符号）。
